...
ПЛАТФОРМЫ И ТРЕБОВАНИЯ К ДЛИНЕ (СТРОГО ОБЯЗАТЕЛЬНО):
${usePlatforms.map(pid => {
  if (pid==="threads") {
    if (useLength.id==="thread_chain") return "• Threads ЦЕПОЧКА: напиши 4-5 тредов. Каждый тред начинается с новой строки и номера: 1/ 2/ 3/ 4/ — до 500 символов каждый (1-3 предложения). Цепочка раскрывает одну тему постепенно. Общий объём 200-350 слов.";
    if (useLength.id==="short") return "• Threads: МАКСИМУМ 50 слов. Один удар — одна мысль.";
    if (useLength.id==="long") return "• Threads: 150-200 слов. Развёрнутый экспертный пост.";
    return "• Threads: МАКСИМУМ 80 слов. Без заголовков. Один абзац.";
  }
  if (pid==="instagram") return "• Instagram: 80-150 слов. Визуальный хук первой строкой.";
  if (pid==="telegram") return "• Telegram: 150-350 слов. Можно структуру. Глубина и механизм.";
  return "• "+pid+": 100-200 слов.";
}).join("\n")}

Для каждой платформы напиши отдельный адаптированный текст нужной длины.
...
- Threads (одиночный): 30-80 слов, хук в первых 2 строках, без заголовков, стадии 0-2
- Threads (цепочка, если length=thread_chain): 4-5 тредов пронумерованных 1/ 2/ 3/ 4/, каждый до 500 символов, 200-350 слов суммарно
