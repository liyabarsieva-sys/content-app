...
{"topics":[{"n":1,"topic":"тема","hook":"хук до 12 слов","quadrant":"Личное + Неожиданное","top":false,"reason":""}]}`;
  ...
  async function generatePlan() {
    ...
    const isQuarter = planPeriod === "quarter";
    const weekTotal = platforms.reduce((sum,pid)=>sum+(planPlatformFreqs[pid]??PLATFORM_FREQ_HINTS[pid]?.rec??3),0);
    const totalPosts = planPeriod === "week" ? weekTotal : planPeriod === "month" ? weekTotal * 4 : weekTotal * 12;
    ...
    const platBreakdown = platforms.map(pid=>{
      ...
      const total = planPeriod==="week" ? freq : planPeriod==="month" ? freq*4 : freq*12;
    ...
    // Sordell interview context for personal topics
    const sordellAnswersText = (sordellAnswers||[]).map((a,i)=>a?.trim()?("- "+a):null).filter(Boolean).join("\n");
    const sordellContext = sordellAnswersText
      ? "\nРЕАЛЬНЫЙ ОПЫТ АВТОРА (используй для личных тем):\n" + sordellAnswersText
      : "";

    // Archetype context
    const archetypeContext = selectedArchetype
      ? `
АРХЕТИП БРЕНДА: ${selectedArchetype.label} — ${selectedArchetype.prompt}`
      : "";

    // Quarterly plan uses a different structure
    const quarterlyInstruction = isQuarter ? `
КВАРТАЛЬНЫЙ ПЛАН — структура по месяцам:
- Месяц 1: создание осознания (акцент на "Не осознаёт проблему" и "Осознаёт проблему")
- Месяц 2: углубление и доверие (акцент на "Ищет решение" и "Выбирает решение")
- Месяц 3: конверсия и лояльность (акцент на "Готов к покупке" + кейсы + продающий контент)
В поле "day" указывай: "Месяц 1, Неделя 1, День 1" и т.д.
Каждый месяц должен развивать нарратив — как сериал с сюжетом.` : "";

    const prompt = `Ты опытный контент-стратег. Составь контент-план на ${planPeriod === "week" ? "1 неделю" : planPeriod === "month" ? "1 месяц" : "3 месяца (квартал)"}.
...
В поле stage используй ТОЧНО эти значения (копируй дословно):
...
${sordellContext ? "ВАЖНО: для квадрантов Личное+Неожиданное и Личное+Известное используй ТОЛЬКО реальные истории автора из раздела выше. Не придумывай личный опыт." : ""}

Для каждого поста укажи платформу из списка: ${platforms.join(", ")}.
Темы должны быть конкретными, цепляющими, на русском языке.
Функция поста: узнавание / перелом / объяснение / усиление / действие
...
{"posts":[{"day":"День 1","platform":"telegram","block":"блок","topic":"конкретная тема","stage":"стадия","sordell":"угол по Сорделл","function":"узнавание"}]}`;
    ...
    try {
      const resp = await fetch("/api/claude", {
        ...
        body:JSON.stringify({ model:"claude-haiku-4-5-20251001", max_tokens:isQuarter ? 8000 : totalPosts > 20 ? 8000 : 5000, messages:[{role:"user",content:prompt}] }),
      });
      const data = await resp.json();
      if (data.error) throw new Error(data.error.message);
      const text = data.content.map(b=>b.text||"").join("");
        ...
        {step===1&&(
          <div>
            <Card>
              ...
              {/* Brand Style Questions */}
              <div style={{marginBottom:14}}>
                <Label text="Стиль бренда" hint="Два вопроса — алгоритм подберёт подходящий стиль автоматически" />

                <div style={{marginBottom:10}}>
                  <div style={{fontSize:11,color:"#5c4e7a",marginBottom:6,fontWeight:500}}>Как вы хотите выглядеть для аудитории?</div>
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {[
                      "как эксперта которому доверяют",
                      "как человека который прошёл через то же",
                      "как того кто говорит правду",
                      "как наставника который ведёт",
                    ].map(opt=>(
                      <button key={opt} onClick={()=>setBrandQ1(brandQ1===opt?"":opt)}
                        style={{padding:"9px 14px",borderRadius:9,border:`1px solid ${brandQ1===opt?"#362d52":"#d8d0e0"}`,background:brandQ1===opt?"#362d52":"#f0eef8",color:brandQ1===opt?"#f4f1ec":"#362d52",fontSize:12,cursor:"pointer",textAlign:"left"}}>
                        {opt}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <div style={{fontSize:11,color:"#5c4e7a",marginBottom:6,fontWeight:500}}>Какой контент вам ближе создавать?</div>
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {[
                      "разборы и факты",
                      ...
                      "закулисье и процесс",
                      "поддержку и истории клиентов",
                    ].map(opt=>(
                      <button key={opt} onClick={()=>setBrandQ2(brandQ2===opt?"":opt)}
                        style={{padding:"9px 14px",borderRadius:9,border:`1px solid ${brandQ2===opt?"#362d52":"#d8d0e0"}`,background:brandQ2===opt?"#362d52":"#f0eef8",color:brandQ2===opt?"#f4f1ec":"#362d52",fontSize:12,cursor:"pointer",textAlign:"left"}}>
                        {opt}
                      </button>
                    ))}
                  </div>
                ...
                {selectedArchetype && (
                  <div style={{marginTop:10,padding:"10px 14px",background:"#362d52",borderRadius:9}}>
                    <div style={{fontSize:11,color:"#e1df2c",fontWeight:700,marginBottom:3}}>✓ Стиль определён: {selectedArchetype.label}</div>
                    <div style={{fontSize:11,color:"rgba(244,241,236,.8)",lineHeight:1.5}}>{selectedArchetype.desc}</div>
                  </div>
                )}
              </div>
